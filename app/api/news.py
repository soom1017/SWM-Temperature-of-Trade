...
@news.get('/list-new/', response_model=List[schemas.NewsBase])
...
@news.get('/list-hot/', response_model=List[schemas.NewsBase])
async def get_hot_news_list(db: Session = Depends(get_db)):
    #TODO: define `hot`, then order by `hot`
    news_list = db.query(models.News).all()
    return news_list

@news.get('/keyword/{keyword_name}', response_model=List[schemas.NewsBase])
