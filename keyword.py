...
class ToDate:
    ...
    def display(self):
        print("md: ",self.md)
class KeyList:
    def __init__(self, from_date, to_date,key_list):
        self.from_date=from_date
        self.to_date=to_date
...
for i in range(len(from_list)):
    ...
    data_list.append(KeyList(from_date,to_date,key_list))
    if count:
        data = pd.DataFrame([(i.from_date,i.to_date,i.key,i.frequency) for i in key_list ],columns=['조회시작','조회종료','키워드','빈도'])
        data.to_csv('C:\\Users\\SeoHyeongSeoksCOM\\Desktop\\new\\key.csv',encoding="utf-8-sig",index=False)
        count=0
    else:
        data = pd.DataFrame([(i.from_date,i.to_date,i.key,i.frequency) for i in key_list ],columns=['조회시작','조회종료','키워드','빈도'])
        data.to_csv('C:\\Users\\SeoHyeongSeoksCOM\\Desktop\\new\\key.csv',encoding="utf-8-sig",index=False,mode='a', header=False)
    data = pd.DataFrame([(i.from_date,i.to_date,i.key,i.frequency) for i in key_list ],columns=['조회시작','조회종료','키워드','빈도'])
    data.to_csv('C:\\Users\\SeoHyeongSeoksCOM\\Desktop\\new\\'+from_date+"_"+to_date+'_key.csv',encoding="utf-8-sig",index=False)
######## Get Network_List #####################
network_list=[]
for key_list in data_list:
    keynetwork_list=[]
    for key in key_list.key_list:
    ...
    data = pd.DataFrame([(i.wrd, i.relateWrd, i.relateCn, i.relateCnt, i.upperRelateWrd, i.brnk, i.secRelateCnt, i.secPer, i.from_date, i.to_date) for i in keynetwork_list ],columns=['wrd','relateWrd','relateCn','relateCnt', 'upperRelateWrd', 'brnk', 'secRelateCnt', 'secPer','from_date','to_date'])
    data.to_csv('C:\\Users\\SeoHyeongSeoksCOM\\Desktop\\new\\'+key_list.from_date+'_'+key_list.to_date+'network.csv',encoding="utf-8-sig",index=False)
    pass
data = pd.DataFrame([(i.wrd, i.relateWrd, i.relateCn, i.relateCnt, i.upperRelateWrd, i.brnk, i.secRelateCnt, i.secPer, i.from_date, i.to_date) for i in network_list ],columns=['wrd','relateWrd','relateCn','relateCnt', 'upperRelateWrd', 'brnk', 'secRelateCnt', 'secPer','from_date','to_date'])
data.to_csv('C:\\Users\\SeoHyeongSeoksCOM\\Desktop\\new\\network.csv',encoding="utf-8-sig",index=False)
pass
##
